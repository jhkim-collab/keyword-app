...
# dedup starts
                ...
                for si in starts:
                    ...
                    for _i_m, _m in enumerate(list(matches)):
                        if getattr(_m, 'start', None) == start_orig and getattr(_m, 'end', 0) < end_orig:
...
def highlight_text(text: str, matches: List[Match2]) -> str:
    ...
    - 각 매칭 구간(start, end) 기준으로만 하이라이트를 적용합니다.
    - 한 구간 안에 여러 kwd_no가 매핑될 수 있으며, 이 경우 리스트로 함께 표기합니다.
    - 리스크 등급에 따라 배경색을 결정합니다.
      * 해당 구간의 등급 집합이 {5}만 있을 때: 연두색 (hl-risk5)
      * 그 외 (1~4, 혹은 1~5 섞임): 노란색 (hl-riskN)
    """
    if not matches:
    ...
    for m in matches:
        ...
        if info is None:
            info = {
                'seg': seg,
                'ids': [],         # 표시할 kwd_no 리스트
                'risk_map': {},    # kwd_no -> grade
            }
            spans[key] = info
        if m.kwd_no:
            if m.kwd_no not in info['ids']:
    ...
    result_parts = []
    cur = 0  # 현재까지 처리한 원본 텍스트 인덱스

    for (start, end), info in ordered:
        if start < cur:
            # 겹치는 구간은 이미 처리된 것으로 보고 건너뜀
            continue
        # 하이라이트 이전의 일반 텍스트
        if cur < start:
            plain_chunk = text[cur:start]
            result_parts.append(html.escape(plain_chunk))

        seg = info['seg']
        ids = info['ids']
        risk_map = info['risk_map']
        ...
        for kid in ids:
            ...
            frag.append(
                f"<span class='kwdno-link' data-kwd='{html.escape(kid)}' style='color:{color}; text-decoration: underline; cursor:pointer'><b>{html.escape(kid)}</b></span>"
            )
        ids_html = ", ".join(frag)

        # 배경색 결정: 등급 집합이 {5}일 때만 연두색, 나머지는 노란색
        grades = {g for g in risk_map.values() if g}
        if grades == {5}:
            cls = "hl-mark hl-risk5"
        else:
            cls = "hl-mark hl-riskN"
        ...
        decorated = f"{leading_ws}<span class='{cls}'>{inner_html}</span>{trailing_ws}"
...
with tab1:
    ...
    if _kwd_candidates:
        # 👉 필터 입력창 내용(list_filter_kwdno_tab1)을 기준으로 선택 상태 계산
        _cur_filter = (st.session_state.get("list_filter_kwdno_tab1") or "").strip()
                        ...
                        if st.button(k, key=f"kwbtn_{k}", type=btn_type):
                            ...
                            if is_selected:
                                ...
                                if k not in parts:
                                    parts.append(k)
                            # 필터 입력창 값 갱신
                            st.session_state["list_filter_kwdno_tab1"] = ", ".join(parts)
        ...
        st.markdown("---")
                # (통합) kwd_no는 아래 "리스트 필터" 영역에서 필터링합니다.
        ...
        try:
            ...
            if qp_val:
                st.session_state["list_filter_kwdno_tab1"] = str(qp_val)
...
# -----------------------------
    ...
    def _reset_list_filters_tab1():
        ...
        st.session_state["list_filter_applied_use_date_tab1"] = False
        st.session_state["list_filter_applied_date_range_tab1"] = None
    ...
    _init_defaults = {
        ...
        "list_filter_kwdno_tab1": "",
        "list_filter_cat_tab1": [],
        "list_filter_risk_tab1": [],
        "list_filter_proof_tab1": [],
        "list_filter_use_date_tab1": False,
        "list_filter_date_range_tab1": None,
        # 적용된 값(Apply 버튼 기준)
        "list_filter_applied_search_tab1": st.session_state.get("list_filter_applied_search_tab1", ""),
        "list_filter_applied_kwdno_tab1": st.session_state.get("list_filter_applied_kwdno_tab1", ""),
        ...
        "list_filter_applied_use_date_tab1": st.session_state.get("list_filter_applied_use_date_tab1", False),
        "list_filter_applied_date_range_tab1": st.session_state.get("list_filter_applied_date_range_tab1", None),
    }
    ...
    with st.form("list_filter_form_tab1", clear_on_submit=False):
        ...
        with f1:
            ...
            kwdno_list = st.text_input(
                "kwd_no 입력(쉼표 구분)",
                placeholder="예: G060,G105,I200",
                key="list_filter_kwdno_tab1",
            )
        ...
        with f5:
            use_date = st.checkbox(
                "마지막 출력일자 필터",
                key="list_filter_use_date_tab1",
            )
            date_range = None
            if use_date:
                # 문자열 timestamp(예: 2025-12-08 17:15:44)를 날짜 범위로 필터링
                _dt_series = pd.to_datetime(_base_df.get("마지막 출력일자", pd.Series([], dtype=str)), errors="coerce")
                _min_dt = _dt_series.min()
                _max_dt = _dt_series.max()
                if pd.notna(_min_dt) and pd.notna(_max_dt):
                    try:
                        date_range = st.date_input(
                            "기간(start~end)",
                            value=(getattr(_min_dt, "date")(), getattr(_max_dt, "date")()),
                            key="list_filter_date_range_tab1",
                        )
                    except Exception:
                        date_range = st.date_input("기간(start~end)", key="list_filter_date_range_tab1")
                else:
                    date_range = st.date_input("기간(start~end)", key="list_filter_date_range_tab1")
    ...
    if apply_clicked:
        ...
        st.session_state["list_filter_applied_kwdno_tab1"] = st.session_state.get("list_filter_kwdno_tab1", "")
        ...
        st.session_state["list_filter_applied_use_date_tab1"] = bool(st.session_state.get("list_filter_use_date_tab1", False))
        st.session_state["list_filter_applied_date_range_tab1"] = st.session_state.get("list_filter_date_range_tab1", None)
    ...
    kwdno_list = st.session_state.get("list_filter_applied_kwdno_tab1", "")
    ...
    use_date = bool(st.session_state.get("list_filter_applied_use_date_tab1", False))
    date_range = st.session_state.get("list_filter_applied_date_range_tab1", None)

    # kwd_no 다중 입력(쉼표 구분) - 정확히 일치하는 항목만 표시
    if kwdno_list and str(kwdno_list).strip():
        _targets = [x.strip().upper() for x in str(kwdno_list).split(",") if x.strip()]
        if _targets and "kwd_no" in _filtered_df.columns:
            _filtered_df = _filtered_df[_filtered_df["kwd_no"].astype(str).str.upper().isin(_targets)]
    ...
    if search_q and str(search_q).strip():
        ...
        _filtered_df = _filtered_df[mask]
    ...
    # 마지막 출력일자 기간 필터
    if use_date and date_range and isinstance(date_range, (list, tuple)) and len(date_range) == 2 and "마지막 출력일자" in _filtered_df.columns:
        d0, d1 = date_range
        if d0 and d1:
            _dt = pd.to_datetime(_filtered_df["마지막 출력일자"], errors="coerce")
            _filtered_df = _filtered_df[(_dt.dt.date >= d0) & (_dt.dt.date <= d1)]

    st.caption(f"현재 표시된 항목: {len(_filtered_df)} / 전체: {len(_base_df)}")
    st.dataframe(_rename_fcol(sort_for_display(_filtered_df[get_db_cols()])), width="stretch")
