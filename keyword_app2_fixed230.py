    ...
    with _pg_conn() as conn:
        ...
        try:
            cur.execute(
                ...
                select
                    kwd_no,
                    coalesce(nullif(keyword_name, ''), nullif("키워드명", '')) as keyword_name,
                    coalesce(nullif(category, ''), nullif("상품카테고리", '')) as category,
                    coalesce(nullif(risk_grade, ''), nullif("리스크 등급", '')) as risk_grade,
                    coalesce(nullif(risk_detail, ''), nullif("리스크 등급별 세부 심의기준", '')) as risk_detail,
                    coalesce(nullif(evidence_type, ''), nullif("증빙자료유형", '')) as evidence_type,
                    coalesce(nullif(alt_keyword, ''), nullif("대체키워드", '')) as alt_keyword,
                    created_at,
                    updated_at
            ...
            # If a statement fails in a transaction, Postgres will abort the transaction
            # and ignore subsequent commands until ROLLBACK. Ensure we reset the state.
            try:
                conn.rollback()
            except Exception:
                pass
            # Fallback: English-only schema
            cur.execute(
                """
        ...
        # Drop English/DB-only columns so UI uses only Korean canonical columns
        for _c in ["keyword_name","category","risk_grade","risk_detail","evidence_type","alt_keyword","created_at","updated_at"]:
            if _c in df.columns:
                df = df.drop(columns=[_c])

        # Ensure expected canonical columns exist
        canonical_order = DB_COLS + ["키워드 등록일자"]
        for c in canonical_order:
            if c not in df.columns:
                df[c] = ""

        # Return only canonical columns for UI (hide DB/meta columns)
