...
def _turso_upsert_keywords(df: pd.DataFrame):
    ...
    # Map from Korean UI columns to DB English columns
    map_ko_to_en = {
        "kwd_no": "kwd_no",
        "키워드명": "keyword_name",
        "상품카테고리": "category",
        ...
        "키워드 조회 수(누적카운트)": "hit_count",
        "마지막 출력일자": "last_shown_at",
        "비고": "note",
        "키워드 등록일자": "created_at",
    }
    # Build records
    ...
    for _, r in df.iterrows():
        recs.append(tuple(r.get(k) for k in map_ko_to_en.keys()))
    sql = f"""
    INSERT INTO {t} (kwd_no, keyword_name, category, risk_grade, alt_keyword, risk_detail, evidence_type, hit_count, last_shown_at, note, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
