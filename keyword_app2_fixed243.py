    ...
    if st.button("텍스트 분석하기", type="primary", key="btn_analyze"):
        try:
            ...
            if st.session_state.kw_df.empty:
                ...
                # (운영) Turso DB가 비어있는 경우 초기 데이터 업로드를 지원
                if _turso_enabled():
                    with st.expander('초기 데이터 업로드 (CSV/XLSX → Turso)', expanded=True):
                        st.caption('Turso DB가 비어있을 때 기존 CSV/XLSX를 한 번에 업로드할 수 있습니다. 업로드 후에는 필터 옵션이 활성화됩니다.')
                        init_mode = st.radio('업로드 방식', ['기존 데이터에 추가/갱신(권장)', '완전 덮어쓰기(주의)'], key='init_upload_mode', horizontal=False)
                        init_file = st.file_uploader('초기 데이터 파일', type=['csv','xlsx'], key='init_uploader')
                        do_init = st.button('초기 데이터 업로드 실행', type='primary', key='btn_init_upload')
                        if do_init:
                            if init_file is None:
                                st.warning('업로드할 파일을 먼저 선택해주세요.')
                            else:
                                try:
                                    name = init_file.name.lower()
                                    raw = init_file.read()
