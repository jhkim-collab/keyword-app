...
def _pg_fetch_keywords_cached() -> pd.DataFrame:
...
def _pg_fast_cache_path(local_csv_path: Path) -> Path:
    ...
    Stored next to the local CSV path so it works both locally and on Streamlit Cloud.
    """
    try:
        return local_csv_path.with_name(f"{local_csv_path.stem}_pgcache.csv")
    except Exception:
...
def load_db(path: Path) -> pd.DataFrame:
    ...
    if _pg_enabled():
        # FAST PATH: try loading a recent local cache first to minimize startup latency.
        cached_df = _pg_try_load_fast_cache(path, max_age_sec=3600)
        if cached_df is not None:
            df = cached_df
            pg_ok = True
        else:
            try:
                df = _pg_fetch_keywords_cached()
                pg_ok = True
                _pg_try_write_fast_cache(path, df)
            except Exception:
                # If we got into a bad connection state, reset once and retry quickly.
                try:
                    _pg_close_cached_conn()
                except Exception:
                    pass
                try:
                    df = _pg_fetch_keywords_cached()
                    ...
                    try:
                        st.error(f"Postgres(DB) 로딩 실패로 로컬 CSV로 대체합니다: {e2}")
                    except Exception:
                        pass
...
if st.sidebar.button("🔄 DB 새로고침(느림)", key="force_refresh_pg"):
    ...
    try:
        # 로컬 캐시도 무효화
        cp = _pg_fast_cache_path(st.session_state.storage_path)
        if cp.exists():
            cp.unlink()
    except Exception:
        pass
    st.sidebar.info("DB 새로고침을 요청했습니다. 페이지를 다시 실행합니다.")
    st.rerun()
