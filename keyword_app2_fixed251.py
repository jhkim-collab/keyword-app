...
try:
    ...
    libsql = None
# ---- DB debug logging (enabled by st.secrets DEBUG_DB_LOG=1 or env DEBUG_DB_LOG=1) ----
def _db_debug_enabled() -> bool:
    try:
        if str(os.environ.get("DEBUG_DB_LOG", "")).strip() in ("1", "true", "True", "YES", "yes"):
            return True
    except Exception:
        pass
    try:
        v = st.secrets.get("DEBUG_DB_LOG", False)
        if isinstance(v, str):
...
def _db_log(msg: str) -> None:
    """Lightweight logger for diagnosing DB/IO latency across reruns."""
    if not _db_debug_enabled():
        return
    ts = time.strftime("%H:%M:%S")
    line = f"[DBDBG {ts}] {msg}"
    # Console (Streamlit Cloud logs / local terminal)
    try:
        print(line, flush=True)
    except Exception:
        pass
    # In-app buffer (optional)
    try:
        buf = st.session_state.setdefault("_dbdbg_lines", [])
        buf.append(line)
        if len(buf) > 300:
            del buf[:100]
    except Exception:
...
def _db_dbg_init_once() -> None:
    if not _db_debug_enabled():
        return
    try:
        st.session_state["_dbdbg_rerun"] = st.session_state.get("_dbdbg_rerun", 0) + 1
        _db_log(f"RERUN #{st.session_state['_dbdbg_rerun']}")
    except Exception:
        _db_log("RERUN (session_state unavailable)")

# ---- Column normalization (Korean UI columns are canonical) ----
_EN_TO_KO_COLMAP = {
    "keyword_name": "키워드명",
    "category": "상품카테고리",
    "risk_grade": "리스크 등급",
    "risk_detail": "리스크 등급별 세부 심의기준",
    "evidence_type": "증빙자료유형",
    "alt_keyword": "대체키워드",
    # Common meta columns seen in Supabase schemas
    "hit_count": "키워드 조회 수(누적카운트)",
    "last_shown_at": "마지막 출력일자",
    "note": "비고",
    "created_at": "키워드 등록일자",
    "updated_at": "업데이트일자",
    "created_by": "등록자",
...
def normalize_keyword_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Normalize dataframe columns so that Korean column names are canonical.
    - If English DB column names are present, they are mapped to Korean and removed.
    - Always removes the duplicate legacy column '키워드등록일자' (no space) if present.
    """
    ...
    # Drop duplicate legacy column (no-space)
    if "키워드등록일자" in df2.columns:
        df2 = df2.drop(columns=["키워드등록일자"])

    # If English columns exist, move values into Korean columns
    for en, ko in _EN_TO_KO_COLMAP.items():
        if en in df2.columns:
            if ko not in df2.columns:
                df2[ko] = df2[en]
            else:
                ko_series = df2[ko].astype(str)
                df2[ko] = df2[ko].where(ko_series.str.strip().ne(""), df2[en])
            df2 = df2.drop(columns=[en])

    # Keep only canonical UI columns to prevent English/Korean duplicates from leaking into the app.
    try:
        allowed = set(DB_COLS + ["키워드 등록일자"])
...
def _pg_params() -> dict:
    ...
    return {
        ...
        "password": password,
        "sslmode": sslmode,
            "connect_timeout": int(_get_secret("DB_CONNECT_TIMEOUT") or 8),
    }

@st.cache_resource(show_spinner=False)
def _pg_get_conn_cached():
    """Create and cache a Postgres connection for the lifetime of the Streamlit process.
    This avoids expensive SSL/TCP handshakes on every rerun.
    """
    params = _pg_params()
    try:
    ...
    try:
        ...
        raise ImportError(
            "Postgres 연동을 위해 psycopg2-binary 또는 psycopg가 필요합니다. requirements.txt에 추가하세요."
        ) from e


def _pg_close_cached_conn() -> None:
    """Close and reset cached connection."""
    try:
        conn = _pg_get_conn_cached()
        try:
            conn.close()
        except Exception:
            pass
...
def _turso_enabled() -> bool:
    return bool(_get_secret("LIBSQL_URL") or _get_secret("TURSO_DATABASE_URL"))

def _turso_params() -> dict:
    # Support common key names
    url = _get_secret("LIBSQL_URL") or _get_secret("TURSO_DATABASE_URL")
    token = _get_secret("LIBSQL_AUTH_TOKEN") or _get_secret("TURSO_AUTH_TOKEN") or _get_secret("TURSO_DATABASE_AUTH_TOKEN")
    table = _get_secret("LIBSQL_TABLE") or "keywords"
    # Local replica file (fast; also avoids OneDrive locks)
    local_db = str(Path.home() / ".olit_keyword_app" / "libsql" / "keywords.db")
    return {"url": url, "token": token, "table": table, "local_db": local_db}

@st.cache_resource(show_spinner=False)
def _turso_conn_cached():
    """Return a cached libsql connection (local replica + sync)."""
    if libsql is None:
        raise RuntimeError("libsql package is not installed. Add 'libsql' to requirements.txt")
    p = _turso_params()
    Path(p["local_db"]).parent.mkdir(parents=True, exist_ok=True)
    # libsql.connect signature supports sync_url/auth_token for replica sync
    return libsql.connect(database=p["local_db"], sync_url=p["url"], auth_token=p["token"])

def _turso_exec(sql: str, params: tuple = ()):
    conn = _turso_conn_cached()
    cur = conn.cursor()
...
def _turso_init_schema():
    p = _turso_params()
    t = p["table"]
    # Keep schema simple & compatible with the app. Use English columns in DB; UI remains Korean via normalization.
    _turso_exec(f"""
    CREATE TABLE IF NOT EXISTS {t} (
        kwd_no TEXT PRIMARY KEY,
        keyword_name TEXT,
        category TEXT,
        risk_grade TEXT,
        risk_detail TEXT,
    ...
    _turso_exec(f"CREATE INDEX IF NOT EXISTS idx_{t}_category ON {t}(category);")
    _turso_exec(f"CREATE INDEX IF NOT EXISTS idx_{t}_risk_grade ON {t}(risk_grade);")
    _turso_exec(f"CREATE INDEX IF NOT EXISTS idx_{t}_keyword_name ON {t}(keyword_name);")
    try:
        _turso_conn_cached().commit()
    except Exception:
        pass

@st.cache_data(ttl=600, show_spinner=False)
def _turso_fetch_keywords_cached() -> pd.DataFrame:
    """Fetch keywords from Turso.

    Returns a dataframe normalized to the app's canonical (Korean) UI columns.
    """
    _turso_init_schema()
    p = _turso_params()
    t = p["table"]
    cur = _turso_exec(
        f"SELECT kwd_no, keyword_name, category, risk_grade, alt_keyword, risk_detail, evidence_type, hit_count, last_shown_at, note, created_at FROM {t}"
    )
    rows = cur.fetchall()
    cols = ["kwd_no","keyword_name","category","risk_grade","alt_keyword","risk_detail","evidence_type","hit_count","last_shown_at","note","created_at"]
    df = pd.DataFrame(rows, columns=cols)
    # Normalize to Korean UI columns and drop any English leftovers
    df = normalize_keyword_columns(df)
    return df

def _turso_invalidate_cache():
    try:
        _turso_fetch_keywords_cached.clear()
    except Exception:
        pass

def _turso_upsert_keywords(df: pd.DataFrame):
    """
    Upsert keywords into Turso (libSQL).

    - Accepts a dataframe that may have Korean UI columns and/or English DB columns.
    - Normalizes to the app's canonical Korean columns first, then maps to English DB columns.
    - Ensures kwd_no exists (auto-generates when missing) and hit_count is an integer (never NULL).
    """
    _turso_init_schema()
    p = _turso_params()
    t = p["table"]

    # Normalize to canonical Korean columns used throughout the app
    df = normalize_keyword_columns(df).copy()

    # Ensure kwd_no exists and is non-empty (PRIMARY KEY)
    if "kwd_no" not in df.columns:
        df["kwd_no"] = ""
    df["kwd_no"] = df["kwd_no"].astype(str).fillna("").map(lambda x: str(x).strip())
    missing_mask = df["kwd_no"].eq("") | df["kwd_no"].str.lower().eq("nan")
    if missing_mask.any():
        df.loc[missing_mask, "kwd_no"] = [f"kwd_{uuid.uuid4().hex[:12]}" for _ in range(int(missing_mask.sum()))]

    # Ensure hit_count is a real integer (Hrana rejects NULL for f64-typed values)
    if "키워드 조회 수(누적카운트)" in df.columns:
        # pandas may keep it as float with NaN -> convert safely
        df["키워드 조회 수(누적카운트)"] = (
            pd.to_numeric(df["키워드 조회 수(누적카운트)"], errors="coerce")
            .fillna(0)
            .astype(int)
        )
    else:
    ...
    # Minimal required columns (create if missing)
    for col in ["키워드명", "상품카테고리", "리스크 등급", "대체키워드", "리스크 등급별 세부 심의기준", "증빙자료유형", "키워드 등록일자"]:
        if col not in df.columns:
            df[col] = ""

    # Map from Korean UI columns to DB English columns (order matters)
    cols_ko = [
        "kwd_no",
        "키워드명",
        "상품카테고리",
    ...
    for opt in ["키워드 마지막 노출 일시", "note"]:
        if opt not in df.columns:
            df[opt] = None

    recs = []
    for _, r in df.iterrows():
        # Normalize text columns: keep None as None; else str
        kwd_no = str(r.get("kwd_no", "")).strip()
        keyword_name = r.get("키워드명", "")
        category = r.get("상품카테고리", "")
        risk_grade = r.get("리스크 등급", "")
        alt_keyword = r.get("대체키워드", "")
        risk_detail = r.get("리스크 등급별 세부 심의기준", "")
        evidence_type = r.get("증빙자료유형", "")
        hit_count = int(r.get("키워드 조회 수(누적카운트)", 0) or 0)
        last_shown_at = r.get("키워드 마지막 노출 일시", None)
        note = r.get("note", None)
        created_at = r.get("키워드 등록일자", None)

        recs.append((
            kwd_no,
            None if keyword_name is None else str(keyword_name),
            None if category is None else str(category),
            None if risk_grade is None else str(risk_grade),
            None if alt_keyword is None else str(alt_keyword),
            None if risk_detail is None else str(risk_detail),
            None if evidence_type is None else str(evidence_type),
            hit_count,
            None if last_shown_at is None else str(last_shown_at),
    ...
    sql = f"""
    INSERT INTO {t} (
        kwd_no, keyword_name, category, risk_grade, alt_keyword, risk_detail,
        evidence_type, hit_count, last_shown_at, note, created_at, updated_at
    )
...
def _turso_clear_table():
    """Delete all rows from the Turso keywords table."""
    _turso_init_schema()
    p = _turso_params()
    t = p["table"]
    _turso_exec(f"DELETE FROM {t}")

def _pg_conn():
    """Context manager returning a cached Postgres connection.

    NOTE: We intentionally do not close the connection here for speed. If the
    connection becomes invalid, callers should call _pg_close_cached_conn().
    """
    conn = _pg_get_conn_cached()
    yield conn
...
@lru_cache(maxsize=1)
def _pg_has_korean_columns() -> bool:
    """Return True if the keywords table contains Korean-named columns.
    This prevents expensive failing SELECTs on every rerun when the DB schema is English-only."""
    try:
        with _pg_conn() as conn:
            ...
            cur.execute(
                ...
                where table_schema = 'public'
                  ...
                  and column_name in ('키워드명','상품카테고리','리스크 등급','리스크 등급별 세부 심의기준','증빙자료유형','대체키워드')
                limit 1
                """
            )
...
@st.cache_data(ttl=600, show_spinner=False)
def _pg_fetch_keywords_cached(refresh_token: int = 0) -> pd.DataFrame:
    """Cached DB fetch to avoid repeated network/DB hits on every Streamlit rerun.

    refresh_token is used only to bust the cache when the user presses 'DB 새로고침'.
    """
    _db_dbg_init_once()
    _t0 = time.perf_counter()
    _db_log(f"pg_fetch_cached start token={refresh_token}")
    df = _pg_fetch_keywords()
    _db_log(f"pg_fetch_cached done in {(time.perf_counter() - _t0) * 1000:.1f}ms rows={len(df)}")
    return df


def _pg_invalidate_cache() -> None:
    # Clear Streamlit cached data
    try:
        _pg_fetch_keywords_cached.clear()
    except Exception:
        pass
    # Clear schema cache
    try:
        _pg_has_korean_columns.cache_clear()  # type: ignore
    except Exception:
        pass


def _pg_fast_cache_path(local_csv_path: Path) -> Path:
    """Path for a fast local cache of DB-loaded keywords.

    IMPORTANT:
    - Do NOT store the cache next to the user's CSV path (often under OneDrive and can be locked),
      as that can cause Permission denied and slow I/O.
    - Store under a dedicated cache directory that is writable both locally and on Streamlit Cloud.
    """
    try:
        import os
        from pathlib import Path
        cache_dir = Path(os.path.expanduser("~")) / ".olit_keyword_app" / "cache"
        cache_dir.mkdir(parents=True, exist_ok=True)
        # Keep the file name stable per logical DB (based on CSV stem for backward compat).
        fname = f"{local_csv_path.stem}_pgcache.csv"
        return cache_dir / fname
    except Exception:
        # Fallback to a simple relative path
        return Path(f"{local_csv_path.stem}_pgcache.csv")


def _pg_try_load_fast_cache(local_csv_path: Path, max_age_sec: int = 3600) -> Optional[pd.DataFrame]:
    """Load cached keywords (Korean canonical columns) if present and fresh."""
    try:
        cp = _pg_fast_cache_path(local_csv_path)
        if not cp.exists():
            return None
        ...
        if df is None or df.empty:
            return None
        # Ensure canonical UI columns only
        df = normalize_keyword_columns(df)
        df = df[[c for c in get_db_cols() + ["키워드 등록일자"] if c in df.columns]].copy()
        return df
    except Exception:
        return None
...
def _pg_fast_cache_age(local_csv_path: Path) -> Optional[float]:
    """Return fast cache age in seconds, or None if missing."""
    try:
        cp = _pg_fast_cache_path(local_csv_path)
        if not cp.exists():
            return None
        return max(0.0, time.time() - cp.stat().st_mtime)
    except Exception:
        return None


def _pg_try_load_fast_cache_any(local_csv_path: Path) -> Optional[pd.DataFrame]:
    """Load fast cache regardless of age (stale-while-revalidate)."""
    try:
        cp = _pg_fast_cache_path(local_csv_path)
        if not cp.exists():
            return None
...
def _pg_background_refresh_fast_cache(local_csv_path: Path, refresh_token: int = 0) -> None:
    """Refresh fast cache in a background thread (best effort).

    This is a speed-first "stale-while-revalidate" strategy:
    - UI loads instantly from the local fast cache (even if stale)
    - We refresh the cache in the background
    - The new data is picked up on the next Streamlit rerun
    """
    try:
        params = _pg_params()
        import psycopg2  # type: ignore
        ...
        try:
            ...
            # Detect Korean columns in DB schema (fresh check, avoids cached connection).
            has_kr = False
            try:
                cur.execute(
                    """
                ...
                has_kr = cur.fetchone() is not None
            except Exception:
                has_kr = False
            ...
            if has_kr:
                cur.execute(
                    ...
                    select
                        kwd_no,
                        coalesce(nullif(keyword_name, ''), nullif("키워드명", '')) as "키워드명",
                        coalesce(nullif(category, ''), nullif("상품카테고리", '')) as "상품카테고리",
                        coalesce(nullif(risk_grade, ''), nullif("리스크 등급", '')) as "리스크 등급",
                        coalesce(nullif(alt_keyword, ''), nullif("대체키워드", '')) as "대체키워드",
                        coalesce(nullif(risk_detail, ''), nullif("리스크 등급별 세부 심의기준", '')) as "리스크 등급별 세부 심의기준",
                        coalesce(nullif(evidence_type, ''), nullif("증빙자료유형", '')) as "증빙자료유형",
                        coalesce(created_at::text, null) as "키워드 등록일자"
                    from public.keywords
                    order by created_at desc nulls last, kwd_no asc
                    """
                )
            else:
                cur.execute(
                    ...
                    select
                        kwd_no,
                        keyword_name as "키워드명",
                        category as "상품카테고리",
                        risk_grade as "리스크 등급",
                        alt_keyword as "대체키워드",
                        risk_detail as "리스크 등급별 세부 심의기준",
                        evidence_type as "증빙자료유형",
                        created_at::text as "키워드 등록일자"
                    from public.keywords
                    order by created_at desc nulls last, kwd_no asc
                    """
                )
            ...
            rows = cur.fetchall()
            _db_log(f"pg_fetch: fetched rows={len(rows)}")
            cols = [d[0] for d in cur.description]
            df = pd.DataFrame(rows, columns=cols)
            if df is None or df.empty:
                return
            _pg_try_write_fast_cache(local_csv_path, df)
        finally:
            try:
                conn.close()
            except Exception:
...
def _pg_try_write_fast_cache(local_csv_path: Path, df: pd.DataFrame) -> None:
    """Write cached keywords for faster startup."""
    try:
        cp = _pg_fast_cache_path(local_csv_path)
        # Always store in canonical Korean form
        df2 = normalize_keyword_columns(df)
        keep_cols = [c for c in get_db_cols() + ["키워드 등록일자"] if c in df2.columns]
        df2 = df2[keep_cols].copy()
        df2.to_csv(cp, index=False, encoding="utf-8-sig")
    except Exception:
        pass
...
def _pg_fetch_keywords() -> pd.DataFrame:
    _db_dbg_init_once()
    _t0 = time.perf_counter()
    _db_log("pg_fetch start")
    """Fetch keywords from Postgres.

    Returns a dataframe normalized to the app's canonical (Korean) UI columns.
    """
    with _pg_conn() as conn:
        cur = conn.cursor()
        # Decide which SELECT to run based on schema, to avoid slow failing queries.
        _t_schema = time.perf_counter()
        _has_ko = _pg_has_korean_columns()
        _db_log(f"pg_fetch: has_korean={_has_ko} schema_check_ms={(time.perf_counter()-_t_schema)*1000:.1f}")
        if _has_ko:
            cur.execute(
                """
                select
            ...
            # English-only schema: alias DB columns directly to Korean UI columns.
            cur.execute(
                """
                select
                    kwd_no,
    ...
    # Normalize and ensure canonical UI columns only
    df = normalize_keyword_columns(df)
    ...
    # Map created_at to Korean register date column if present
    if "created_at" in df.columns:
        if "키워드 등록일자" not in df.columns:
            df["키워드 등록일자"] = df["created_at"]
        else:
            ko_series = df["키워드 등록일자"].astype(str)
            df["키워드 등록일자"] = df["키워드 등록일자"].where(ko_series.str.strip().ne(""), df["created_at"])

    # UI should only expose canonical columns (no English/meta duplicates)
    canonical_order = list(DB_COLS)
    if "키워드 등록일자" in df.columns and "키워드 등록일자" not in canonical_order:
        canonical_order.append("키워드 등록일자")

    for c in canonical_order:
        if c not in df.columns:
            df[c] = ""

    _db_log(f"pg_fetch done total_ms={(time.perf_counter()-_t0)*1000:.1f} rows={len(df)}")
...
def _rename_fcol(df):
    ...
    - 이미 DISPLAY_F_COL이 존재하는데 RAW_F_COL도 존재하면(중복 위험),
      UI 표시를 위해 RAW_F_COL을 제거하고 DISPLAY_F_COL만 남깁니다.
    """
    try:
        import pandas as _pd

        def _apply_rename(_df):
            # 중복 방지: 이미 DISPLAY가 있으면 RAW는 UI에서 숨김
            if hasattr(_df, "columns") and (RAW_F_COL in _df.columns) and (DISPLAY_F_COL in _df.columns):
                return _df.drop(columns=[RAW_F_COL])
            if hasattr(_df, "rename") and hasattr(_df, "columns"):
                return _df.rename(columns={RAW_F_COL: DISPLAY_F_COL})
            return _df

        # DataFrame (가급적 이 분기로)
        if hasattr(df, "rename") and hasattr(df, "columns"):
            return _apply_rename(df)

        # pandas Styler 처리
        if getattr(df, "__class__", None).__name__ == "Styler":
            base = getattr(df, "data", None)
            if base is not None and hasattr(base, "rename"):
                return _apply_rename(base).style

        return df
    except Exception:
        return df

def _safe_df_for_st(df):
    """st.dataframe/pyarrow 변환 안정화: 중복 컬럼 제거/정리."""
    try:
        import pandas as _pd
        if df is None:
            return df
        ...
        # pyarrow가 싫어하는 object(리스트/딕트 등) 방어적으로 문자열화
        for c in _df.columns:
            if _df[c].dtype == object:
                sample = _df[c].dropna().head(20).tolist()
...
def load_db(path: Path) -> pd.DataFrame:
    _db_dbg_init_once()
    _t0 = time.perf_counter()
    _db_log(f"load_db start path={path}")
    # Turso/libSQL (preferred) as source of truth when configured
    if _turso_enabled():
        try:
            df = _turso_fetch_keywords_cached()
            _db_log(f"load_db: turso rows={len(df)} elapsed_ms={(time.perf_counter()-_t0)*1000:.1f}")
            return df
        except Exception as e:
            try:
                st.error(f"Turso(DB) 로딩 실패로 로컬 CSV로 대체합니다: {e}")
            except Exception:
                pass
    ...
    if _pg_enabled():
        # SPEED-FIRST:
        # - Always load instantly from the local fast cache if it exists (even if stale)
        # - Refresh the cache in the background when stale or when user requests refresh
        pg_ok = False

        cache_df = _pg_try_load_fast_cache_any(path)
        cache_age = _pg_fast_cache_age(path)

        # If we have any cache, use it immediately to avoid blocking the UI.
        if cache_df is not None:
            df = cache_df
            pg_ok = True

            # Kick off background refresh if cache is stale (default 10 minutes) or refresh was requested.
            stale = (cache_age is None) or (cache_age > 600)
            refresh_requested = bool(st.session_state.get("pg_force_refresh", False))
            if (stale or refresh_requested) and not st.session_state.get("pg_bg_refresh_running", False):
                st.session_state.pg_bg_refresh_running = True
                st.session_state.pg_force_refresh = False

                def _runner():
                    try:
                        _pg_background_refresh_fast_cache(path, int(st.session_state.get("pg_refresh_token", 0)))
                    finally:
                        # Mark as completed; next rerun will read updated cache.
                        st.session_state.pg_bg_refresh_running = False

                threading.Thread(target=_runner, daemon=True).start()

        # If cache is missing, do a single blocking fetch (first-run cost).
        if not pg_ok:
            try:
                df = _pg_fetch_keywords_cached(int(st.session_state.get("pg_refresh_token", 0)))
                pg_ok = True
                _pg_try_write_fast_cache(path, df)
            except Exception as e:
                try:
    ...
    # 중복 컬럼 정리: '키워드등록일자'(띄어쓰기 없음) 제거
    if '키워드등록일자' in df.columns:
        df = df.drop(columns=['키워드등록일자'])

    # English header CSV / mixed schemas: normalize to canonical UI columns
    df = normalize_keyword_columns(df)
...
def _ensure_kwd_no(df: pd.DataFrame) -> pd.DataFrame:
    """Ensure kwd_no exists and is non-empty (required as primary key in Turso).

    If kwd_no is missing or blank, generate stable unique IDs.
    """
    df = df.copy()
    if "kwd_no" not in df.columns:
        df["kwd_no"] = ""

    # Normalize to string, preserve existing non-empty values
    ser = df["kwd_no"].astype("string")
    mask = ser.isna() | (ser.str.strip() == "")
    if mask.any():
        import uuid
        # Generate unique IDs
        df.loc[mask, "kwd_no"] = [f"kwd_{uuid.uuid4().hex[:12]}" for _ in range(int(mask.sum()))]
    return df
...
def save_db(df: pd.DataFrame, path: Path) -> None:
    ...
    # English header CSV / mixed schemas: normalize to canonical UI columns
    df = normalize_keyword_columns(df)
    df = _ensure_kwd_no(df)
    # If Turso/libSQL is configured, persist to Turso.
    if _turso_enabled():
        try:
            _turso_upsert_keywords(df)
            _turso_invalidate_cache()
            return
        except Exception as e:
            try:
                st.error(f"Turso(DB) 저장 실패로 로컬 CSV로 대체합니다: {e}")
            except Exception:
                pass
...
def init_state():
    ...
    if "pg_refresh_token" not in st.session_state:
        st.session_state.pg_refresh_token = 0
...
def find_matches(text: str, kw_df: pd.DataFrame, match_mode: str = "유사 키워드 포함", exact_relaxed: bool = False, suffix_suggestions: dict = None, **kwargs) -> List[Match2]:
    ...
    * '정확 일치' 모드는 기존 substring 기반 동작을 유지합니다.
    ...
    # --- Perf: index tokens for fast lookup in fuzzy mode ---
    token_pos_lower = {}
    for _i_tok, _t_tok in enumerate(tokens_lower):
        token_pos_lower.setdefault(_t_tok, []).append(_i_tok)
    # Collect 1-token Korean keyword stems for deferred (inverted) suffix/prefix matching
    korean_stem_to_infos = {}  # stem(compact) -> list of (term, category, risk, detail, kwd_no)
    try:
        _ko_suffix_whitelist = set(get_ko_suffix_whitelist())
    except Exception:
        _ko_suffix_whitelist = set()
    try:
        _ko_suffix_max_len = max((len(s) for s in _ko_suffix_whitelist), default=0)
    except Exception:
        _ko_suffix_max_len = 0
    ...
    for r in records:
        ...
        if match_mode == "유사 키워드 포함":
            for v in variants:
                ...
                # 1) 정확한 토큰 시퀀스 일치
                # 1) 정확한 토큰 시퀀스 일치 (속도 개선: 토큰 인덱스 활용)
                starts = []
                if n == 1:
                    starts = list(token_pos_lower.get(v_tokens_lower[0], []))
                else:
                    _first = v_tokens_lower[0]
                    for i in token_pos_lower.get(_first, []):
                        if i + n <= len(tokens_lower) and tokens_lower[i:i+n] == v_tokens_lower:
                            starts.append(i)
                ...
                # Perf: 영어 토큰에 대해서만 수행(한글 키워드에 대해 전체 윈도우 스캔을 방지)
                _is_eng_tok = lambda t: bool(re.fullmatch(r"[a-z0-9\-]+", str(t)))
                is_english_variant = all(_is_eng_tok(t) for t in v_tokens_lower)

                def _eng_forms(tok: str):
                    base = _normalize_english_token(tok)
                    if not base:
                        return []
                    forms = {base}
                    if len(base) >= 3:
                        forms.update({base + "s", base + "es", base + "ed", base + "ing", base + "er", base + "est"})
                ...
                if is_english_variant:
                    if n == 1:
                        # 단일 토큰은 형태변화 후보를 사전 인덱스에서 바로 조회 (전체 window 스캔 금지)
                        for _f in _eng_forms(v_tokens_lower[0]):
                            starts.extend(token_pos_lower.get(_f, []))
                    else:
                        # 구(phrase)는 첫 토큰 형태변화 후보 위치만 후보로 잡아 제한 스캔
                        cand = set()
                        for _f in _eng_forms(v_tokens_lower[0]):
                            cand.update(token_pos_lower.get(_f, []))
                        for i2 in sorted(cand):
                            if i2 + n > len(tokens_lower):
                                continue
                            ...
                            for j2 in range(n):
                                ...
                                if stem_len <= 0:
                                    ok = False
                                    break
                                # highlight는 공유 stem 부분만 표시(가장 앞 토큰만 override)
                                if local_override is None:
                                    try:
                                        _s0 = token_spans[i2 + j2][1]
                                        local_override = (_s0, _s0 + stem_len)
                                    except Exception:
                                        pass
                            if ok:
                                starts.append(i2)
                ...
                if is_korean_kw:
                    # Perf: defer Korean stem/suffix/prefix composite matching to a single inverted pass
                    # to avoid scanning all tokens for every keyword.
                    korean_stem_to_infos.setdefault(v_compact, []).append((term, category, risk, detail, kwd_no))
...
# dedup starts
    ...
    # --- Perf: deferred Korean composite matching (single pass over tokens) ---
    if match_mode == '유사 키워드 포함' and korean_stem_to_infos:
        try:
            _is_hangul = lambda s: bool(s) and all(('가' <= ch <= '힣') for ch in s)
            for _ti, _tok in enumerate(tokens_compact):
                if not _is_hangul(_tok):
                    continue
                _tok_len = len(_tok)
                if _tok_len < 3:
                    continue
                _t_start, _t_end = token_spans[_ti][1], token_spans[_ti][2]
    
                # (3) 조사/어미 화이트리스트 기반: token = stem + suffix
                if _ko_suffix_max_len:
                    _max = min(_ko_suffix_max_len, _tok_len - 2)
                    for _suf_len in range(1, _max + 1):
                        _suf = _tok[-_suf_len:]
                        if _suf not in _ko_suffix_whitelist:
                            continue
                        _stem = _tok[:-_suf_len]
                        infos = korean_stem_to_infos.get(_stem)
                        if not infos:
                            continue
                        _s = _t_start
                        _e = _t_start + len(_stem)
                        if _e > _t_end:
                            _e = _t_end
                        for (_term, _cat, _risk, _detail, _kwd_no) in infos:
                            if _e <= _s:
                                continue
                            _range_occupied = any((i in occupied_set) for i in range(_s, _e))
                            if not _range_occupied:
                                matches.append(Match2(term=_term, start=_s, end=_e, category=_cat, risk=_risk, detail=_detail, kwd_no=_kwd_no))
                                for _j in range(_s, _e):
                                    occupied_set.add(_j)
                            else:
                                _same_span_exists = any((m.start == _s and m.end == _e) for m in matches)
                                _already_same = any((m.start == _s and m.end == _e and m.term == _term and m.category == _cat and m.risk == _risk) for m in matches)
                                if _same_span_exists and not _already_same:
                                    matches.append(Match2(term=_term, start=_s, end=_e, category=_cat, risk=_risk, detail=_detail, kwd_no=_kwd_no))
    
                # (4) 합성어 내부 접미(끝부분) 매칭: token.endswith(keyword), prefix len 1..6
                for _pre_len in range(1, 7):
                    if _pre_len >= _tok_len - 1:
                        break
                    _kw = _tok[_pre_len:]
                    if len(_kw) < 2:
                        continue
                    infos = korean_stem_to_infos.get(_kw)
                    if not infos:
                        continue
                    _e = _t_end
                    _s = max(_t_start, _e - len(_kw))
                    for (_term, _cat, _risk, _detail, _kwd_no) in infos:
                        if _e <= _s:
                            continue
                        _range_occupied = any((i in occupied_set) for i in range(_s, _e))
                ...
                # (5) 합성어 내부 접두(앞부분) 매칭: token.startswith(keyword), suffix len 1..6
                for _suf_len in range(1, 7):
                    if _suf_len >= _tok_len - 1:
                        break
                    _kw = _tok[:-_suf_len]
                    if len(_kw) < 2:
                        continue
                    ...
                    if not infos:
                        continue
                    _s = _t_start
                    _e = min(_t_end, _t_start + len(_kw))
                    for (_term, _cat, _risk, _detail, _kwd_no) in infos:
                        if _e <= _s:
                            continue
                        _range_occupied = any((i in occupied_set) for i in range(_s, _e))
...
# 속도 최우선: 기본적으로는 로컬 캐시를 우선 사용합니다.
# DB의 최신 데이터를 강제로 다시 불러오고 싶다면 아래 버튼을 누르세요(재로딩이 느릴 수 있음).
if st.sidebar.button("🔄 DB 새로고침(느림)", key="force_refresh_pg"):
    # Speed-first: keep showing cached data instantly, and refresh DB cache in the background.
    st.session_state.pg_refresh_token = int(st.session_state.get("pg_refresh_token", 0)) + 1
    st.session_state.pg_force_refresh = True
    try:
        _pg_invalidate_cache()
    except Exception:
        pass
    st.sidebar.info("DB 새로고침을 요청했습니다. 캐시 데이터로 먼저 표시한 뒤 백그라운드로 갱신합니다.")
    st.rerun()
...
# -----------------------------
# 초기 데이터 업로드 (Turso 사용 시 권장)
# -----------------------------
if st.session_state.kw_df.empty:
    st.info("키워드 DB가 비어 있습니다. CSV/XLSX 파일로 초기 데이터를 업로드할 수 있습니다.")
    with st.expander("📥 초기 데이터 업로드 (CSV/XLSX → Turso)", expanded=True):
        st.caption("동시 사용자 환경을 위해 원본 데이터는 Turso(DB)에 저장됩니다. 파일 업로드는 최초 1회(또는 재적재) 용도입니다.")
        init_mode = st.radio(
            "업로드 방식",
            ["추가/갱신(Upsert, 권장)", "완전 덮어쓰기(테이블 비우고 업로드)"],
            index=0,
            key="init_upload_mode_top",
        )

        up = st.file_uploader(
            "CSV 또는 XLSX 파일 업로드",
            type=["csv", "xlsx"],
            key="init_upload_file_top",
        )

        # 업로드 파일은 rerun 시 사라질 수 있으므로 session_state에 보관
        if up is not None:
            try:
                st.session_state._init_upload_buf = {
                    "name": up.name,
                    "bytes": up.getvalue(),
                }
            except Exception:
                st.session_state._init_upload_buf = None

        buf = st.session_state.get("_init_upload_buf")
        if buf and buf.get("name"):
            st.write(f"선택된 파일: `{buf['name']}`")
        else:
            st.caption("파일을 선택하면 여기 표시됩니다.")

        def _read_init_upload_df(_buf):
            name = (_buf.get("name") or "").lower()
            b = _buf.get("bytes") or b""
            bio = io.BytesIO(b)
            if name.endswith(".xlsx"):
                return pd.read_excel(bio)
            # CSV: 인코딩 자동(utf-8-sig 우선, 실패 시 cp949)
            try:
                bio.seek(0)
        ...
        if st.button("초기 데이터 업로드 실행", type="primary", key="run_init_upload_top", disabled=not (buf and buf.get("bytes"))):
            try:
                inc_df = _read_init_upload_df(buf)
                inc_df = normalize_keyword_columns(inc_df)
                inc_df = _ensure_kwd_no(inc_df)

                # 기본 컬럼 방어 (kwd_no 없으면 생성)
                if "kwd_no" not in inc_df.columns:
                    inc_df["kwd_no"] = ""

                if init_mode.startswith("완전 덮어쓰기"):
                    _turso_clear_table()

                save_db(inc_df, Path(st.session_state.storage_path))
                st.session_state.kw_df = load_db(Path(st.session_state.storage_path))
                # Verify DB row count (best-effort)
                try:
                    if _turso_enabled():
                        _turso_init_schema()
                        _cnt = _turso_exec(f"SELECT COUNT(*) FROM {_turso_params()['table']}").fetchone()[0]
                        st.info(f"Turso 테이블 저장 확인: 현재 총 {_cnt}건")
                except Exception as _e:
                    st.warning(f"Turso 저장 확인 실패(무시 가능): {_e}")
                scan_existing_counters()
                st.success(f"초기 데이터 업로드 완료: {len(inc_df)}건 처리했습니다.")
                # 업로드 버퍼는 유지(원하면 재시도 가능)하되, 화면 갱신
                st.rerun()
            except Exception as e:
                st.error(f"초기 데이터 업로드 중 오류: {e}")
...
""",
    ...
    if st.button("텍스트 분석하기", type="primary", key="btn_analyze"):
        try:
            ...
            if not st.session_state.kw_df.empty:
                suffix_suggestions = {}
                ...
                if matches:
                    ...
                    # Perf: 하이라이트 미리보기는 렌더링 비용이 크므로, 매우 긴 텍스트는 앞부분만 표시합니다.
                    preview_text = text
                    try:
                        _MAX_PREVIEW_CHARS = 30000
                        if isinstance(text, str) and len(text) > _MAX_PREVIEW_CHARS:
                            preview_text = text[:_MAX_PREVIEW_CHARS]
                            st.info(f"하이라이트 미리보기는 앞 {_MAX_PREVIEW_CHARS:,}자만 표시합니다. (속도 최적화)")
                    except Exception:
                        preview_text = text
                    try:
                        preview_matches = [m for m in matches if getattr(m, 'end', 0) <= len(preview_text)]
                    except Exception:
                        preview_matches = matches
                    st.session_state['analysis_highlight_html'] = highlight_text(preview_text, preview_matches)
                    st.session_state['analysis_text_preview'] = preview_text
                ...
                # 키워드 DB가 비어 있으면 분석을 수행할 수 없습니다.
                st.warning("키워드 DB가 비어 있어 텍스트 분석을 수행할 수 없습니다. (DB 등록 후 다시 시도해주세요)")
                st.session_state['analysis_show'] = False
                st.session_state['analysis_df_full'] = None
                st.session_state['analysis_df_display_unique'] = None
                st.session_state['analysis_highlight_html'] = ''
...
# 적용된 값(Apply 버튼 기준)
    ...
    # NOTE: Avoid KeyError when DB column list includes columns missing from _filtered_df.
    # We keep the display column order from get_db_cols(), and fill missing columns with NaN.
    db_cols = get_db_cols()
    missing_cols = [c for c in db_cols if c not in _filtered_df.columns]
    if missing_cols:
        try:
            st.warning(f"DB cols missing in dataframe (filled with NaN): {missing_cols}")
        except Exception:
            pass
    _list_view_df = sort_for_display(_filtered_df.reindex(columns=db_cols))
    ...
    if edit_mode_effective and is_admin():
        ...
        st.dataframe(_safe_df_for_st(_rename_fcol(_list_view_df)), width="stretch")
